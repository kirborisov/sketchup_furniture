...
# ═══════════════════════════════════════════════════════════════════
# ПРИМЕР: изменить параметры конкретного шкафа и перерисовать
# ═══════════════════════════════════════════════════════════════════
#
# 1) Найти шкаф по имени в описании кухни и изменить его параметры (ширина,
#    наполнение и т.д.). Объект Cabinet один и тот же — меняем поля/конфиг.
#
# 2) Вызвать rebuild_cabinet("Имя шкафа") — удалится старый корпус, ящики,
#    двери и размеры в зоне этого шкафа, затем построится новый на том же месте.
#
# Пример: сделать «Шкаф 5, ящики» уже (600 вместо 700) и с двумя ящиками:
#
#   cab = $kitchen.lower_cabinets.find { |c| c.name == "Шкаф 5, ящики" }
#   cab.instance_variable_set(:@width, 600)
#   cab.instance_variable_get(:@drawers_config).clear
#   cab.instance_variable_set(:@drawers_positions, [0, 400])  # 2 ящика: 0–400 мм и 400–верх
#   $kitchen.rebuild_cabinet("Шкаф 5, ящики")
#
# Пример: только перерисовать без смены параметров (например после ручных правок):
#
#   $kitchen.rebuild_cabinet("Шкаф 5, ящики")
#
# После пересборки при необходимости обнови размеры и инструмент:
#
#   $kitchen.show_dimensions(:sections)
#   $kitchen.activate_drawer_tool
